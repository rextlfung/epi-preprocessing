%{
run_bart.m — CG-SENSE reconstruction of all temporal frames via BART pics.

Reads the zero-filled k-space produced by run_preprocessing.m, loads (or recomputes)
...
%% ── Configuration ────────────────────────────────────────────────────────────
run('./config.m');                              % Loads session-level cfg
cfg = set_seq_paths(cfg, cfg.seqnames{1});      % Resolve per-sequence paths (single-sequence mode)
